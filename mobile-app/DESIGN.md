...
## 2. Colors: Tonal Architecture Single Source of Truth (SSOT)

> [!IMPORTANT]
> The absolute single source of truth for all colors in this application is `constants/colors.js`.
> There must NEVER be any duplicate color hex codes in other config files, stylesheets, or components. `tailwind.config.js` intelligently ingests `constants/colors.js` and builds NativeWind CSS variables automatically. If you need a color in JS for React Navigation, import `constants/colors.js` or use the `useThemeColor` hook. If you need a color for styling, use standard Tailwind classes (like `bg-surface-container`), which dynamically adapt to light and dark modes based on the generated CSS variables.
