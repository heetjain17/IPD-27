...
const CATEGORIES = [
  { id: '1', title: 'Food & Drink', color: 'bg-[#4d5f00]', textColor: 'text-on-primary' },
  {
    id: '2',
    title: 'Arts & Culture',
  ...
  { id: '4', title: 'Nightlife', color: 'bg-[#4d5f00]', textColor: 'text-on-primary' },
  {
    id: '5',
    title: 'Shopping',
    color: 'bg-surface-container-highest',
        ...
        {isSearching ? (
          ...
          <View className="px-5 pb-6 pt-2">
            {SUGGESTIONS.map((s) => (
              <Pressable
                key={s.id}
                className={`flex-row items-center gap-3 border-b border-outline-variant/20 py-4 ${
                  s.highlight ? 'border-l-2 border-l-primary pl-3' : ''
                }`}
              >
                <IconSearch size={18} color="#adaaaa" />
          ...
          <View className="pb-8">
            ...
            <View className="mb-8 px-5">
              ...
              <View className="mt-2 rounded-xl border border-outline-variant/10 bg-surface-container-low px-4 py-2">
                {TRENDING_SEARCHES.map((t, i) => (
                  <Pressable
                    key={i}
                    className={`flex-row items-center justify-between py-3 ${
                      i !== TRENDING_SEARCHES.length - 1 ? 'border-b border-outline-variant/10' : ''
                    }`}
                  >
                    <View className="flex-row items-center gap-3">
                    ...
                    <IconChevronRight size={16} color="#adaaaa" />
                  </Pressable>
                ))}
              </View>
            </View>
            ...
            <View className="px-5 pb-6">
              ...
              <View className="mt-3 flex-row flex-wrap justify-between gap-y-3">
                {CATEGORIES.map((cat) => (
                  <Pressable
                    key={cat.id}
                    className={`h-24 w-[48%] justify-end rounded-2xl p-4 ${cat.color} border border-outline-variant/10`}
                  >
