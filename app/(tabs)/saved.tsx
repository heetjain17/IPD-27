    ...
    <ScreenWrapper>
        ...
        <View className="pb-8">
          ...
          >
            {COLLECTIONS.map((c) => (
              <Pressable
                key={c.id}
                className={`h-52 w-40 justify-between rounded-3xl border border-outline-variant/10 p-4 ${c.bgColor}`}
              >
                <View className="h-8 w-8 items-center justify-center rounded-full bg-black/20">
                  <IconBookmarkFilled size={14} color="#ffffff" />
                </View>
                <View>
      ...
      <View className="absolute bottom-6 left-5 right-5">
        <Pressable
          ...
          style={{ shadowColor: '#d7fd4e', shadowOpacity: 0.2, shadowRadius: 10 }}
        >
          <IconPlus size={20} color="#4d5f00" strokeWidth={2.5} />
